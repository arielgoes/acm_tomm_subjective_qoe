...
Reads credentials from the environment (or --url / --key):
    ...
    SUPABASE_SECRET_KEY   the SECRET key (sb_secret_...) -- bypasses RLS so it can
                          read the append-only table. Keep it secret, server-side
                          only. A legacy service_role JWT also works and may be
                          passed via SUPABASE_SERVICE_KEY.

Usage:
    SUPABASE_URL=... SUPABASE_SECRET_KEY=... python export_responses.py
    python export_responses.py --url https://xxxx.supabase.co --key <secret_key>
"""
import argparse
import csv
import io
...
def auth_headers(key):
    """Headers for a Supabase REST request.

    The key always goes in `apikey`. Legacy JWT keys (service_role) also belong
    in the Authorization header; new secret keys (sb_secret_...) must NOT be sent
    as a Bearer token, so we only add it for JWT-style keys.
    """
    headers = {"apikey": key}
...
def main():
    ...
    if not args.url or not args.key:
        print("Error: set SUPABASE_URL and SUPABASE_SECRET_KEY (or pass "
